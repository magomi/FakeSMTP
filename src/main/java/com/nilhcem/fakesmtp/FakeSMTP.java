...
import com.nilhcem.fakesmtp.check.CheckHttpHandler;
...
public final class FakeSMTP {
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            //      FAKESMTP_CHECKPORT and
            //      FAKESMTP_CONTEXT
            // where the http check endpoint listens for connections
            Server server = new Server(Integer.valueOf(System.getenv("FAKESMTP_CHECKPORT")));
            ContextHandler context = new ContextHandler(System.getenv("FAKESMTP_CONTEXT"));
            context.setHandler(new CheckHttpHandler());
            ...
            EventQueue.invokeLater(new Runnable() {
                @Override public void run() {
                    try {
                        ...
                        LOGGER.debug(
                                "Error: {} - This is probably because we run on a non-Mac platform and these components are not implemented",
                                e.getMessage());
                    } catch (Exception e) {
                        LOGGER.error("", e);
                    }
